-- 0008_add_source_url_to_current_deals.sql
-- Agrega source_url (de sailings) a la vista materializada current_deals
-- para que el frontend pueda enlazar directamente a la oferta en cruceros.cl.
-- Requiere drop+recreate porque PostgreSQL no permite ALTER MATERIALIZED VIEW ADD COLUMN.

-- 1. Eliminar índices dependientes primero
drop index if exists idx_current_deals_unique;
drop index if exists idx_current_deals_score;
drop index if exists idx_current_deals_region;

-- 2. Eliminar la vista materializada
drop materialized view if exists current_deals;

-- 3. Recrear con source_url incluido
create materialized view current_deals as
select
  s.id                as sailing_id,
...
-- 4. Recrear índices
create unique index idx_current_deals_unique
  on current_deals (sailing_id, cabin_type);
create index idx_current_deals_score
  on current_deals (deal_score desc);
create index idx_current_deals_region
  on current_deals (region, deal_score desc);

-- 5. Mantener permiso de lectura pública (ya otorgado en 0007)
grant select on current_deals to anon, authenticated;
