...
class FakeRpcQuery:
    ...
    def __init__(self, fn: str, params: dict[str, Any], log: list[dict[str, Any]]) -> None:
        self._fn = fn
        self._params = params
        self._log = log
...
class FakeSupabase:
    ...
    def rpc(self, fn: str, params: dict[str, Any] | None = None) -> FakeRpcQuery:
        return FakeRpcQuery(fn, params or {}, self.calls)
...
def test_process_inserts_into_all_tables(sailing: CruiseSailing) -> None:
    ...
    tables_touched = [c["table"] for c in fake.calls if "table" in c]
    rpcs_called = [c["fn"] for c in fake.calls if "fn" in c]
    ...
    assert rpcs_called == ["upsert_price_history_daily"]
...
def test_cache_avoids_duplicate_upserts_for_same_sailing(sailing: CruiseSailing) -> None:
    ...
    # Catalogue tables upserted once each. price_history inserted twice, RPC called twice.
    ...
    rpcs = [c for c in fake.calls if "fn" in c]
    assert len([c for c in rpcs if c["fn"] == "upsert_price_history_daily"]) == 2
...
def test_different_cabin_same_sailing_reuses_caches(sailing: CruiseSailing) -> None:
    ...
    # Same itinerary + sailing, only price_history grows. RPC called twice.
    assert len(_table_calls(fake.calls, "cruise_lines")) == 1
    assert len(_table_calls(fake.calls, "itineraries")) == 1
    assert len(_table_calls(fake.calls, "sailings")) == 1
    assert len(_table_calls(fake.calls, "price_history")) == 2
